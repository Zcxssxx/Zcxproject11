...
- `moon publish --dry-run` reached the registry and returned `409 Conflict` for version `0.1.0`
- that duplicate-version response indicates `Zcxssxx/moon-ninja@0.1.0` is already registered on Mooncakes, so the current work is publication hardening rather than first-time publication
